...
/**
 * An input format that reads the first KEY_LENGTH characters of each line as the key
 * and the rest of the line as the value. Both key and value are represented
 * as Text.
 */
public class MSTInputFormat extends FileInputFormat<Text,Text> {
  ...
  static final int KEY_LENGTH = 50;
  ...
  public static void writePartitionFile(final JobContext job, 
    ...
    final MSTInputFormat inFormat = new MSTInputFormat();
  ...
  static class MSTRecordReader extends RecordReader<Text,Text> {
    ...
    public MSTRecordReader() throws IOException {
    }
  ...
  public List<InputSplit> getSplits(JobContext job) throws IOException {
    ...
    if (job.getConfiguration().getBoolean(MSTScheduler.USE, true)) {
      MSTScheduler scheduler = new MSTScheduler(
        lastResult.toArray(new FileSplit[0]), job.getConfiguration());
      lastResult = scheduler.getNewFileSplits();
      t3 = System.currentTimeMillis(); 
      System.out.println("Spent " + (t3 - t2) + "ms computing MSTScheduler splits.");
    }
    return lastResult;
  }
}
